...
export async function POST(request: Request) {
  try {
    ...
    const challenge = await getAssertionChallenge(input.challengeId, userId, 'envelope');
    if (response.id !== challenge.credential.id) {
      throw new Error('assertion used a different passkey than the envelope challenge');
    }
    ...
    if (!aad.equals(expectedAad)) throw new Error('encrypted key envelope is bound to the wrong identity');
    ...
    if (ciphertext.length < 33 || ciphertext.length > 512) {
      throw new Error('envelope ciphertext has an invalid length');
    }
