...
confusion attack and a consensus-invalid recovery path) and how the current
design fixes them.
...
Everything is `npm run <command> [-- args]`. Offline: `setup`, `cooperative`,
...
- **Cooperative exit is trust-minimized in structure, not yet in ceremony.**
  The aggregate key is standard BIP-327, but the demo signer reconstructs the
  aggregate secret from all three personal keys in one process. A production
  deployment must replace `signCooperativeExitPsbt` with an interactive
  MuSig2 session (two-round nonce exchange, e.g. wallets implementing
  BIP-327) so no machine ever holds all keys. The on-chain artifacts are
  identical.
- **The recovery leaf is an N-1 collusion path after the delay.** Once a vault
  ...
  amounts, raise `RECOVERY_DELAY_BLOCKS` substantially and treat the leaf as
  the explicit trust trade-off it is.
...
- **Deterministic demo keys.** All keys derive from `VAULT_DEMO_SEED`. The
  default seed is public; live commands refuse it, but even with a strong
  seed, one seed generating all participants' keys is a demo convenience —
  real participants each generate their own keys on their own machines.
- **Signet only.** The amounts, fee constants, and network are pinned to
  signet. Nothing here is mainnet-ready without an interactive MuSig2
  ceremony, per-participant key custody, and a longer recovery delay.
