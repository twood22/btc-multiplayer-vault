...
export function verifyVaultTransaction({ txHex, prevouts }) {
  ...
  tx.ins.forEach((input, index) => {
    ...
    let witness = input.witness.map((item) => Buffer.from(item));
    ...
    if (witness.length === 1) {
      verifyKeyPathSpend({ tx, index, prevoutScripts, prevoutValues, outputKey, witness, checks });
    } else {
      verifyScriptPathSpend({ tx, index, prevoutScripts, prevoutValues, outputKey, witness, checks });
    }
  });
...
function verifyKeyPathSpend({ tx, index, prevoutScripts, prevoutValues, outputKey, witness, checks }) {
...
function verifyScriptPathSpend({ tx, index, prevoutScripts, prevoutValues, outputKey, witness, checks }) {
  ...
  checks.push(`input ${index}: control block commits leaf to output key (merkle depth ${(controlBlock.length - 33) / 32})`);
  ...
  if (
    ...
    executeSingleKeyLeaf({ tx, index, prevoutScripts, prevoutValues, leafHash, decompiled, stack, checks });
    return;
  }
  executeRecoveryLeaf({ tx, index, prevoutScripts, prevoutValues, leafHash, decompiled, stack, checks });
}

function executeSingleKeyLeaf({ tx, index, prevoutScripts, prevoutValues, leafHash, decompiled, stack, checks }) {
  if (stack.length !== 1) {
  ...
  if (!ecc.verifySchnorr(sighash, decompiled[0], signature)) {
    throw new Error(`input ${index} tapscript signature is invalid for leaf key`);
  }
  checks.push(`input ${index}: pk(${decompiled[0].toString('hex')}) OP_CHECKSIG satisfied`);
}

function executeRecoveryLeaf({ tx, index, prevoutScripts, prevoutValues, leafHash, decompiled, stack, checks }) {
...
function parseSchnorrSignature(item, label) {
...
function decodeScriptNum(value, label) {
  ...
  if (!Buffer.isBuffer(value) || value.length === 0 || value.length > 5) {
    throw new Error(`${label} is not a minimally encoded script number`);
  }
  return bitcoin.script.number.decode(value);
}

function compactSize(length) {
...
export function runBip327KeyAggVectors() {
