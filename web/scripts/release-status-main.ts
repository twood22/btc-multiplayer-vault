...
import { esploraEnabled } from '../../src/esplora';
...
import { RELEASE_NETWORK, RELEASE_MANUAL_GATES, assertExplicitOperatorNetwork } from '../../src/release-network';

assertExplicitOperatorNetwork();
...
const manualGates = RELEASE_MANUAL_GATES;

try {
  const proofDigest = process.env[RELEASE_NETWORK.proofDigestEnv] || '';
  const receipt = readProtectedLiveSigbashProofReceipt(
    process.env[RELEASE_NETWORK.proofReceiptEnv] || RELEASE_NETWORK.proofReceiptPath,
    proofDigest,
  );
  verifiedProofDigest = receipt.proofDigest;
  checks.push(check(
    `protected live Sigbash ${RELEASE_NETWORK.network} proof receipt is present and matches its reviewed digest`,
    true,
...
checks.push(check(
  `tiny-${RELEASE_NETWORK.network} amount is explicit and within the private-beta cap`,
...
checks.push(check(
  `${RELEASE_NETWORK.network} recovery delay is explicit and positive`,
  recoveryDelay !== null && recoveryDelay > 0 && recoveryDelay <= 65_535,
  ...
  if (!databaseEndpoint.ok) {
    ...
    try {
      const restoreReceipt = readProtectedDatabaseRestoreReceipt(
        process.env.DATABASE_RESTORE_RECEIPT || RELEASE_NETWORK.restoreReceiptPath,
        {
    ...
    try {
      ...
      const states = await sql<Array<{
        ...
        matching_network_keys: number;
        matching_network_rosters: number;
        ...
        SELECT
          ...
          (SELECT count(*)::integer FROM participant_sigbash_keys
            WHERE network = ${RELEASE_NETWORK.network}) AS matching_network_keys,
          (SELECT count(*)::integer FROM vault_rosters
            WHERE network = ${RELEASE_NETWORK.network}
              AND artifact_json->>'network' = ${RELEASE_NETWORK.network}
              AND status = 'confirmed') AS matching_network_rosters,
      ...
      checks.push(check('the immutable roster has nine live Sigbash keys and three confirmations',
        state.live_sigbash_keys === 9 && state.roster_confirmations === 3 &&
          state.matching_network_keys === 9 && state.matching_network_rosters === 1,
        `${state.matching_network_keys}/9 ${RELEASE_NETWORK.network} keys; ` +
          `${state.matching_network_rosters}/1 confirmed ${RELEASE_NETWORK.network} roster; ` +
          `${state.roster_confirmations}/3 confirmations`));
...
if (process.env.BITCOIN_BACKEND || process.env.BITCOIN_RPC_URL) {
  try {
    ...
    checks.push(check(`configured Bitcoin backend identifies as ${RELEASE_NETWORK.network}`,
      !esploraEnabled() && info.chain === BITCOIN_NETWORK_CONFIG.coreChain,
      esploraEnabled() ? 'initial funding requires the private Bitcoin Core path' : `${info.blocks} blocks`));
  } catch (error) {
    checks.push(check(`configured Bitcoin backend identifies as ${RELEASE_NETWORK.network}`, false, safeError(error)));
  }
} else {
  checks.push(check(`${RELEASE_NETWORK.network} Bitcoin backend is configured`, false));
}
...
const reportBody = {
  ...
  deploymentGate: {
    ...
    organizationCommand: RELEASE_NETWORK.network === 'mainnet'
      ? 'npm run sigbash-proof-org-id' : 'npm run signet:proof-org-id',
    setupCommand: RELEASE_NETWORK.network === 'mainnet'
      ? 'SIGBASH_MODE=live npm run live-predeployment-setup' : 'npm run signet:predeployment-setup',
    proofCommand: RELEASE_NETWORK.network === 'mainnet'
      ? 'SIGBASH_MODE=live npm run live-predeployment-proof' : 'npm run signet:predeployment-proof',
  },
