...
type ParticipantId = 'alice' | 'bob' | 'carol';

interface ParticipantBrowser {
  id: ParticipantId;
...
const participants: ParticipantId[] = ['alice', 'bob', 'carol'];
...
async function createParticipantBrowser(input: {
  browser: Browser;
  baseURL: string;
  id: ParticipantId;
  ...
  await page.getByLabel('Your name').fill(`${capitalize(input.id)} Cooperative Browser`);
...
async function seedPublicSigbashRegistrations(
...
function syntheticMainnetXpub(label: string): string {
  const root = deterministicKeypair('cooperative-browser-acceptance', `${label}:root`);
