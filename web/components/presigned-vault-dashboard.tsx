...
import { PresignedCashoutPanel } from './presigned-cashout';
...
export function PresignedVaultDashboard({ member, status, identity }: {
  ...
  return <main className="shell dashboard-shell">
    <header className="brand-row"><div className="brand-mark" aria-hidden="true">₿</div>
      <div className="brand-copy"><strong>{member.vaultName}</strong><span>Bitcoin Multiplayer Vault · Presigned v{status.version}</span></div>
    ...
    <PresignedReadiness vaultId={status.vaultId} protocol={status.protocol} />
    ...
    {member.setupComplete && member.recoveryComplete && identity && <PresignedRuntime vaultId={status.vaultId} protocol={status.protocol}
    ...
    {member.setupComplete && member.recoveryComplete && identity && <PresignedFees vaultId={status.vaultId} protocol={status.protocol}
      ownIdentity={{ id: status.participantId, personalPublicKeyHex: identity.personalPublicKeyHex,
        payoutXonlyPublicKeyHex: identity.payoutXonlyPublicKeyHex }} passkeys={member.passkeys}
      chainConfig={presignedBrowserChainConfig()} requiredConfirmations={chainConfirmationsRequired()} />}
    {member.setupComplete && member.recoveryComplete && identity && <PresignedCashoutPanel vaultId={status.vaultId} protocol={status.protocol}
      ownIdentity={{ id: status.participantId, personalPublicKeyHex: identity.personalPublicKeyHex,
        payoutXonlyPublicKeyHex: identity.payoutXonlyPublicKeyHex }} passkeys={member.passkeys}
      chainConfig={presignedBrowserChainConfig()} requiredConfirmations={chainConfirmationsRequired()} />}
  </main>;
