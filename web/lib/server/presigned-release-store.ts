...
import { PARTICIPANT_IDS, PRESIGNED_PROTOCOL, PRESIGNED_PROTOCOL_V3, type PresignedProtocol, type ParticipantId } from '../../../src/presigned/types';
import { assertPresignedFundingRestoreBinding, validatePresignedFundingRestoreReceipt } from '../../../src/presigned/restore';
import { assert, commitmentDigest, exactKeys, hexBytes, identifier, sameCanonical, presignedDomain, validatePresignedProtocol } from '../../../src/presigned/validation';
...
export function assertPresignedSoftwareRelease(protocol: PresignedProtocol = PRESIGNED_PROTOCOL) {
  assert(process.env[protocol === PRESIGNED_PROTOCOL_V3 ? 'PRESIGNED_V3_MAINNET_AUTHORIZATION' : 'PRESIGNED_V2_MAINNET_AUTHORIZATION'] === 'separately-approved-mainnet-spending' &&
  ...
  return inspectPresignedSoftwareEvidence(protocol);
}
export function inspectPresignedSoftwareEvidence(protocol: PresignedProtocol = PRESIGNED_PROTOCOL) {
  ...
  assert(build.protocol === protocol && build.network === BITCOIN_NETWORK_NAME, 'release build protocol/network differs');
  ...
  assert(receipt.protocol === protocol && receipt.receiptDigest === required('PRESIGNED_V2_ACCEPTANCE_RECEIPT_DIGEST') && receipt.sourceDigest === build.sourceDigest &&
...
export async function getPresignedFundingReadiness(userId: string) {
  ...
  await check('tested-software', 'Exact protocol, source, OCI image, offline utility, reviewed automated acceptance and real default-Signet evidence', () => inspectPresignedSoftwareEvidence(status.protocol));
  ...
  await check('ceremony', 'Complete graph, all solo and required fixed-refund authorizations, and every participant’s offline plus two-passkey restoration receipts', () =>
  ...
  if (BITCOIN_NETWORK_NAME === 'mainnet') {
    await check('separate-mainnet-authorization', 'Separate explicit mainnet activation, not granted by the implementation goal', () =>
      assert(process.env[status.protocol === PRESIGNED_PROTOCOL_V3 ? 'PRESIGNED_V3_MAINNET_AUTHORIZATION' : 'PRESIGNED_V2_MAINNET_AUTHORIZATION'] === 'separately-approved-mainnet-spending' &&
  ...
  return { version: status.version, protocol: status.protocol, network: BITCOIN_NETWORK_NAME,
 ...
 */
export function assertPresignedFundingSignatureRelease(kind: string, protocol: PresignedProtocol = PRESIGNED_PROTOCOL) {
  if (BITCOIN_NETWORK_NAME === 'mainnet' && ['begin-wallet-signing','submit-funding-signature','approve-funding'].includes(kind))
    assertPresignedSoftwareRelease(protocol);
}
...
export async function recordPresignedFundingReleaseUse(input: {
  ...
  await db()`INSERT INTO presigned_funding_release_uses(vault_id,protocol,epoch_id,broadcast_intent_id,fee_package_id,
    release_report_digest,acceptance_receipt_digest,finalization_digest)
    VALUES (${input.vaultId}::uuid,${evidence.protocol},${input.epochId}::uuid,${input.broadcastIntentId}::uuid,${input.feePackageId}::uuid,
...
export async function assertPresignedFundingRelease(vaultId: string, epochId: string) {
  ...
  assert(epoch.graph, 'release requires a frozen graph');
  const receipt = assertPresignedSoftwareRelease(epoch.graph.protocol);
  ...
  assert(report.protocol === epoch.graph.protocol && receipt.protocol === epoch.graph.protocol, 'release evidence protocol differs from the funded graph');
  ...
  return { protocol: epoch.graph.protocol, reportDigest: report.reportDigest, acceptanceReceiptDigest: receipt.receiptDigest,
...
export async function inspectPresignedFundingPrerequisites(vaultId: string, epochId: string) {
  ...
  exactKeys(build, ['version','protocol','network','sourceDigest'], 'presigned build identity');
  validatePresignedProtocol(build.version, build.protocol);
  assert(build.network === BITCOIN_NETWORK_NAME, 'release build network differs');
  hexBytes(build.sourceDigest, 32, 'build source digest');
  ...
  const frozen = await transaction(async sql => {
    await sql`SELECT id FROM vaults WHERE id = ${vaultId}::uuid AND protocol = ${build.protocol} FOR UPDATE`;
    const rows = await sql<Array<{ state_json: PresignedCeremonyState; state_digest: Buffer }>>`
      SELECT state_json, state_digest FROM presigned_ceremonies WHERE vault_id = ${vaultId}::uuid AND protocol = ${build.protocol}`;
    const state = rows[0]?.state_json;
    assert(state && state.protocol === build.protocol && commitmentDigest(presignedDomain(state.protocol, 'ceremony/state'), state) === rows[0]!.state_digest.toString('hex'),
    ...
    for (const receipt of epoch.backups) validatePresignedRestorationReceipt({ graph: epoch.graph,
      preauthorizations: epoch.preauthorizations, ...(epoch.recoveryAuthorizations ? { recoveryAuthorizations: epoch.recoveryAuthorizations } : {}),
