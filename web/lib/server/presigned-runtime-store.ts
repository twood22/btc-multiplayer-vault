...
import { PRESIGNED_PROTOCOL_V3, isPresignedProtocol, type PresignedProtocol, type RecoveryAuthorization, type ParticipantId, type PresignedGraph, type PresignedPublicKit } from '../../../src/presigned/types.js';
import { assert, canonicalJson, commitmentDigest, exactKeys, identifier, safeInteger, sameCanonical, presignedDomain, presignedVersion, validatePresignedProtocol } from '../../../src/presigned/validation.js';
...
type KitSnapshot = { graph: PresignedGraph | null; preauthorizations: PresignedPublicKit['preauthorizations']; recoveryAuthorizations?: RecoveryAuthorization[] };
interface StateRow { id: string; vault_id: string; protocol: PresignedProtocol; epoch_id: string; graph_digest: Buffer;
...
export async function getPresignedRuntimeStatus(userId: string) {
  return transaction(async sql => {
    ...
    const epochs = await sql<Array<{ epoch_id: string; status: string; graph_digest: Buffer; snapshot_json: KitSnapshot }>>`
      SELECT epoch_id,status,graph_digest,snapshot_json FROM presigned_funding_epochs
      WHERE vault_id=${membership.vaultId}::uuid AND protocol=${membership.protocol} AND graph_digest IS NOT NULL ORDER BY ordinal
    `;
    const kits = epochs.filter(row => row.snapshot_json.preauthorizations.length === 12 &&
      (membership.protocol !== PRESIGNED_PROTOCOL_V3 || row.snapshot_json.recoveryAuthorizations?.length === 9)).map(row => ({
    ...
    const rows = await sql<StateRow[]>`
      SELECT id,vault_id,protocol,epoch_id,graph_digest,proposal_digest,proposal_json,state_json,state_digest
      FROM presigned_runtime_proposals WHERE vault_id=${membership.vaultId}::uuid AND protocol=${membership.protocol}
    ...
    return { version: presignedVersion(membership.protocol), ...membership,
...
export async function createPresignedRuntimeActionChallenge(input: {
  ...
  return transaction(async sql => {
    ...
    const rows = await sql<Array<{ id: string; expires_at: Date }>>`
      INSERT INTO presigned_runtime_action_challenges(vault_id,protocol,user_id,participant_id,credential_id,credential_counter,
        proposal_id,proposal_digest,proposal_json,action_json,action_digest,challenge,expires_at)
      VALUES (${membership.vaultId}::uuid,${membership.protocol},${input.userId}::uuid,${membership.participantId},${credential.id},${credential.counter},
    ...
    return { id: rows[0]!.id, expiresAt: rows[0]!.expires_at.toISOString(), challenge: input.challenge,
      vaultId: membership.vaultId, participantId: membership.participantId, protocol: membership.protocol,
...
export async function completePresignedRuntimeAction(challenge: PresignedRuntimeActionChallenge, newCounter: number,
  ...
  return transaction(async sql => {
    ...
    assert(challenge.protocol === membership.protocol && membership.vaultId === challenge.vaultId && membership.participantId === challenge.participantId,
    ...
    if (current.action.kind === 'contribute-nonce') {
      await sql`INSERT INTO presigned_runtime_nonce_commitments(proposal_id,vault_id,protocol,user_id,participant_id,public_nonce,contribution_json)
        VALUES (${next.proposal.proposalId}::uuid,${membership.vaultId}::uuid,${membership.protocol},${current.credential.userId}::uuid,
          ${membership.participantId},${hex(current.action.publicNonce.pubnonce)},${sql.json(json(current.action.publicNonce))})`;
    }
    await sql`INSERT INTO presigned_runtime_action_events(challenge_id,vault_id,protocol,user_id,participant_id,credential_id,
      proposal_id,action_digest,action_json,resulting_state_digest)
      VALUES (${current.id}::uuid,${membership.vaultId}::uuid,${membership.protocol},${current.credential.userId}::uuid,${membership.participantId},
        ${current.credential.id},${next.proposal.proposalId}::uuid,${hex(current.actionDigest)},${sql.json(json(current.action))},
        ${hex(commitmentDigest(presignedDomain(next.protocol, 'runtime/state'),next))})`;
    return { version: next.version, protocol: next.protocol, vaultId: membership.vaultId,
...
async function evaluate(sql: TransactionSql, membership: Membership, action: PresignedRuntimeAction,
  ...
  let kit: PresignedPublicKit;
  assert(action.protocol === membership.protocol, 'runtime action protocol differs from immutable vault');
  ...
  if (action.kind === 'create-proposal') {
    kit = await loadKit(sql, membership.vaultId, action.epochId, action.graphDigest, membership.protocol); graph = kit.graph;
    ...
    const rows = await sql<StateRow[]>`SELECT id,vault_id,protocol,epoch_id,graph_digest,proposal_digest,proposal_json,state_json,state_digest
      FROM presigned_runtime_proposals WHERE id=${lookupId}::uuid AND vault_id=${membership.vaultId}::uuid AND protocol=${membership.protocol}`;
    assert(rows.length === 1, 'runtime proposal is missing from this vault');
    state = readState(rows[0]!); proposal = state.proposal;
    kit = await loadKit(sql, membership.vaultId, proposal.epochId, proposal.graphDigest, membership.protocol); graph = kit.graph;
...
async function loadKit(sql: TransactionSql, vaultId: string, epochId: string, graphDigest: string, protocol: PresignedProtocol): Promise<PresignedPublicKit> {
  const rows = await sql<Array<{ snapshot_json: KitSnapshot }>>`
    SELECT snapshot_json FROM presigned_funding_epochs WHERE vault_id=${vaultId}::uuid AND epoch_id=${epochId}::uuid
      AND protocol=${protocol} AND graph_digest=${hex(graphDigest)}
  `;
  ...
  const kit = validatedKit(vaultId, epochId, graphDigest, rows[0]!.snapshot_json);
  assert(kit.protocol === protocol, 'runtime kit differs from retained epoch protocol');
  return kit;
}
function validatedKit(vaultId: string, epochId: string, graphDigest: string,
  snapshot: KitSnapshot): PresignedPublicKit {
  ...
  assert(graph.protocol === PRESIGNED_PROTOCOL_V3 || !Object.hasOwn(snapshot, 'recoveryAuthorizations'), 'legacy epoch cannot contain V3 approvals');
  return createPresignedPublicKit({ graph, preauthorizations: snapshot.preauthorizations,
    ...(graph.protocol === PRESIGNED_PROTOCOL_V3 ? { recoveryAuthorizations: snapshot.recoveryAuthorizations ?? [] } : {}) });
}
function readState(row: StateRow): PresignedRuntimeState {
  ...
  assert(state.protocol === row.protocol && state.proposal.protocol === row.protocol && state.proposal.proposalId === row.id &&
  ...
  assert(commitmentDigest(presignedDomain(state.protocol, 'runtime/state'),state) === row.state_digest.toString('hex'), 'persisted runtime state digest changed');
...
async function persistState(sql: TransactionSql, vaultId: string, state: PresignedRuntimeState): Promise<void> {
  ...
  await sql`INSERT INTO presigned_runtime_proposals(id,vault_id,protocol,epoch_id,graph_digest,proposal_digest,source_txid,source_vout,
    slot,kind,status,proposal_json,state_json,state_digest,transaction_digest,final_txid,transaction_hex)
    VALUES (${p.proposalId}::uuid,${vaultId}::uuid,${state.protocol},${p.epochId}::uuid,${hex(p.graphDigest)},${hex(p.digest)},${hex(p.source.txid)},
      ${p.source.vout},${p.slot},${p.kind},${state.status},${sql.json(json(p))},${sql.json(json(state))},
      ${hex(commitmentDigest(presignedDomain(state.protocol, 'runtime/state'),state))},${completed ? hex(completed.transactionDigest) : null},
...
async function membershipForUser(sql: TransactionSql, userId: string, lock: boolean): Promise<Membership> {
  ...
  `;
  assert(rows.length === 1 && isPresignedProtocol(rows[0]!.protocol), 'exactly one known presigned runtime vault membership is required');
...
async function loadChallenge(sql: TransactionSql, userId: string, id: string, lock: boolean): Promise<PresignedRuntimeActionChallenge> {
  ...
  const rows = await sql<Array<{ id: string; vault_id: string; protocol: PresignedProtocol; participant_id: ParticipantId; credential_id: string; credential_counter: string;
    ...
    SELECT a.id,a.vault_id,a.protocol,a.participant_id,a.credential_id,a.credential_counter::text,a.action_json,a.action_digest,
    ...
    WHERE a.id=${id}::uuid AND a.user_id=${userId}::uuid AND a.protocol IN ('presigned-graph-v2','presigned-graph-v3')
  ...
  assert(action.protocol === row.protocol && row.proposal_json.protocol === row.protocol && actionDigest === row.action_digest.toString('hex') && row.proposal_json.proposalId === action.proposalId &&
    row.proposal_json.digest === row.proposal_digest.toString('hex'), 'runtime challenge changed action or proposal digest');
  const credential = await selectedCredential(sql,userId,row.credential_id,{ vaultId: row.vault_id, participantId: row.participant_id, vaultStatus: 'unknown', protocol: row.protocol });
  assert(credential.counter === Number(row.credential_counter), 'runtime passkey counter changed after challenge');
  return { id: row.id, vaultId: row.vault_id, participantId: row.participant_id, protocol: row.protocol,
