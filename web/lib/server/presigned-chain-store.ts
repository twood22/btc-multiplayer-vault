...
import { type PresignedProtocol, type PresignedGraph } from '../../../src/presigned/types';
import { assert, canonicalJson, commitmentDigest, identifier, sameCanonical, presignedDomain, presignedVersion } from '../../../src/presigned/validation';
...
interface EpochRow {
  vault_id: string; protocol: PresignedProtocol; epoch_id: string; snapshot_json: PresignedFundingEpoch; snapshot_digest: Buffer;
...
export async function loadPresignedFundingEpoch(vaultId: string, epochId: string,
  ...
  const rows = await connection<Array<{ protocol: PresignedProtocol; snapshot_json: PresignedFundingEpoch; snapshot_digest: Buffer; graph_digest: Buffer | null }>>`
    SELECT protocol, snapshot_json, snapshot_digest, graph_digest FROM presigned_funding_epochs
    WHERE vault_id = ${vaultId}::uuid AND epoch_id = ${epochId}::uuid AND protocol IN ('presigned-graph-v2','presigned-graph-v3')
  `;
  const row = rows[0];
  assert(row && row.snapshot_json.epochId === epochId &&
    commitmentDigest(presignedDomain(row.protocol, 'ceremony/epoch'), row.snapshot_json) === row.snapshot_digest.toString('hex'),
  'retained epoch is missing or changed');
  if (row.snapshot_json.graph) {
    const graph = validatePresignedGraph(row.snapshot_json.graph);
    assert(graph.protocol === row.protocol && graph.digest === row.graph_digest?.toString('hex') && graph.roster.vaultId === vaultId &&
...
export async function getPresignedChainStatus(userId: string) {
  ...
  const memberships = await db()<Array<{ vault_id: string; protocol: PresignedProtocol }>>`
    SELECT m.vault_id, v.protocol FROM vault_members m JOIN vaults v ON v.id = m.vault_id
    WHERE m.user_id = ${userId}::uuid AND v.protocol IN ('presigned-graph-v2','presigned-graph-v3')
  `;
  assert(memberships.length === 1, 'one known presigned vault membership is required');
  ...
  `;
  return { version: presignedVersion(memberships[0]!.protocol), protocol: memberships[0]!.protocol, vaultId,
...
export async function pollPresignedVaultChains(dependencies: {
  ...
  const rows = await db()<EpochRow[]>`
    SELECT e.vault_id, e.protocol, e.epoch_id, e.snapshot_json, e.snapshot_digest, e.graph_digest,
    ...
    WHERE e.protocol IN ('presigned-graph-v2','presigned-graph-v3') AND e.graph_digest IS NOT NULL
  ...
  for (const [vaultId, epochs] of grouped) {
    ...
    try {
      for (const row of epochs) {
        ...
        assert(commitmentDigest(presignedDomain(row.protocol, 'ceremony/epoch'), epoch) === row.snapshot_digest.toString('hex') &&
        ...
        if (row.chain_state) assert(current.protocol === row.protocol && commitmentDigest(presignedDomain(row.protocol, 'watch/state'), current) === row.chain_digest?.toString('hex'), 'watched state digest changed');
      ...
      await transaction(async sql => {
        const locked = await sql<Array<{ id: string }>>`SELECT id FROM vaults
          WHERE id = ${vaultId}::uuid AND protocol = ${epochs[0]!.protocol} FOR UPDATE`;
        ...
        const currentEpochs = await sql<Array<{ epoch_id: string; snapshot_digest: Buffer; graph_digest: Buffer }>>`
          ...
          WHERE vault_id = ${vaultId}::uuid AND protocol = ${epochs[0]!.protocol} AND graph_digest IS NOT NULL
          ORDER BY ordinal
        `;
        ...
        for (const item of prepared) {
          ...
          const nextDigest = commitmentDigest(presignedDomain(item.graph.protocol, 'watch/state'), nextState);
          await sql`
            INSERT INTO presigned_chain_states (vault_id, protocol, epoch_id, graph_digest, state_json, state_digest,
              snapshot_json, last_success_at, deferred_reason)
            VALUES (${vaultId}::uuid, ${item.graph.protocol}, ${item.row.epoch_id}::uuid, ${bytes(item.graph.digest)}, ${sql.json(json(nextState))},
          ...
          if (result && (result.addedTxids.length || result.invalidatedTxids.length || result.reanchoredTxids.length)) {
            await sql`INSERT INTO presigned_chain_events (vault_id, epoch_id, previous_state_digest,
              ...
              VALUES (${vaultId}::uuid, ${item.row.epoch_id}::uuid, ${bytes(commitmentDigest(presignedDomain(item.graph.protocol, 'watch/state'), item.current))},
        ...
        if (!deferredReason) {
          ...
          await sql`UPDATE vaults SET status = ${nextStatus} WHERE id = ${vaultId}::uuid AND protocol = ${epochs[0]!.protocol}`;
        }
      });
  ...
  return { protocols: [...new Set(rows.map(row => row.protocol))], vaults: results, deferredVaults: results.filter(item => item.deferredReason !== null).length };
}
