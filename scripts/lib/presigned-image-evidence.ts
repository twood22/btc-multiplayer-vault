...
export async function validateRetainedImageEvidence(directory: string, sourceDigest: string, network: 'signet' | 'mainnet',
  expectedProtocol: PresignedProtocol = PRESIGNED_PROTOCOL_V3) {
  ...
  const protocol = receipt.protocol as PresignedProtocol;
  assert(protocol === expectedProtocol && receipt.kind === `presigned-v${presignedVersion(protocol)}-exact-oci-execution` &&
  ...
  assert.equal(commitmentDigest(presignedDomain(protocol, 'exact-oci-execution'), body), receiptDigest, 'retained image execution receipt changed');
  ...
  for (const command of receipt.commands) {
    const expected = imageExecutionCommand(command.stage, network, receipt.executionDirectory, image.configDigest, protocol);
    assert.deepEqual(command.command, expected, 'retained image evidence substituted an execution command or environment');
    assert.equal(command.commandDigest, commitmentDigest(presignedDomain(protocol, 'image-command'), expected));
  ...
  assert(identity.build?.sourceDigest === sourceDigest && identity.build.network === network && identity.build.protocol === protocol &&
    identity.build.version === presignedVersion(protocol) && identity.network?.version === 1 && identity.network.network === network &&
  ...
  assert.equal(browser.protocol, protocol, 'retained browser evidence belongs to a different protocol');
  ...
  return { receiptDigest: receipt.receiptDigest as string, sourceDigest, network, protocol, testedImageManifestDigest: image.manifestDigest,
